...
This code takes two input i.e, name of the CHGCAR file and the direction and returns charge density of all planes and the name of the CHGCAR file.
...
224 are the total number of planes for which the density is calculates, and (480,60) is the dimension of charge density for each plane.To extarct X positions, Y positions and denisty, 
...
"""
def Charge_densities_from_CHGCAR_all_planes(name_of_the_file, direction):
    ...
    for value in z_values:
        ...
        xarray=np.zeros((ngridpts[iplane1],ngridpts[iplane2]),np.float32)
        yarray=np.zeros((ngridpts[iplane1],ngridpts[iplane2]),np.float32)
    ...
    print("Shape of the charge density array is:" + str(np.shape(charge_density_for_all_planes)))
...
import numpy as np
np.savetxt('density.txt',charge_density[0]) #where 0 is the plane number. It can be changed accordingly.
